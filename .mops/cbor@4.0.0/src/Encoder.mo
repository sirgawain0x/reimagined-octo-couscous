...
module {
  ...
  public func toBytes(value : Types.Value) : Result.Result<[Nat8], Types.EncodingError> {
    ...
    switch (toBytesBuffer(Buffer.fromList(list), value)) {
  ...
  ///
  /// Returns:
  /// * `#ok`: Successfully encoded the value
  /// * `#err(Types.EncodingError)`: Encoding failed with error details
  ///
  /// Example:
  /// ```motoko
  ...
  /// };
  /// ```
  public func toBytesBuffer(buffer : Buffer.Buffer<Nat8>, value : Types.Value) : Result.Result<(), Types.EncodingError> {
    switch (value) {
  ...
  /// ```
  public func toBytesType0(buffer : Buffer.Buffer<Nat8>, value : Nat64) : Result.Result<(), Types.EncodingError> {
  ...
  /// ```
  public func toBytesType1(buffer : Buffer.Buffer<Nat8>, value : Int) : Result.Result<(), Types.EncodingError> {
    let maxValue : Int = -1;
  ...
  /// ```
  public func toBytesType2(buffer : Buffer.Buffer<Nat8>, value : [Nat8]) : Result.Result<(), Types.EncodingError> {
  ...
  /// let result = toBytesType3(Buffer.fromList(buffer), "Hello, CBOR!");
  /// // buffer now contains the encoded CBOR for text string "Hello, CBOR!"
  /// ```
  public func toBytesType3(buffer : Buffer.Buffer<Nat8>, value : Text) : Result.Result<(), Types.EncodingError> {
  ...
  /// ```
  public func toBytesType4(buffer : Buffer.Buffer<Nat8>, value : [Types.Value]) : Result.Result<(), Types.EncodingError> {
  ...
  /// ```
  public func toBytesType5(buffer : Buffer.Buffer<Nat8>, value : [(Types.Value, Types.Value)]) : Result.Result<(), Types.EncodingError> {
    let arrayLength : Nat64 = Nat64.fromNat(value.size());
  ...
  /// ```
  public func toBytesType6(buffer : Buffer.Buffer<Nat8>, tag : Nat64, value : Types.Value) : Result.Result<(), Types.EncodingError> {
  ...
  /// ```
  public func toBytesType7(buffer : Buffer.Buffer<Nat8>, value : { #integer : Nat8; #bool : Bool; #_null; #_undefined; #float : FloatX.FloatX }) : Result.Result<(), Types.EncodingError> {
    let (additionalBits : Nat8, additionalBytes : ?Iter.Iter<Nat8>) = switch (value) {
      ...
      case (#integer(i)) {
        if (i <= 19) {
          ...
          return #err(#invalidValue("Major Type 7 integer must be <= 19 or >= 32"));
        } else {
          (24 : Nat8, ?Iter.singleton<Nat8>(i));
        };
      };
      case (#float(f)) {
        let floatBytesBuffer = List.empty<Nat8>();
        FloatX.toBytesBuffer(Buffer.fromList(floatBytesBuffer), f, #msb);
        let n : Nat8 = switch (f.precision) {
          case (#f16) 25;
          case (#f32) 26;
          case (#f64) 27;
        };
        (n, ?List.values(floatBytesBuffer));
      };
    };
  ...
  private func encodeRaw(
    ...
    majorType : Nat8,
    additionalBits : Nat8,
    additionalBytes : ?Iter.Iter<Nat8>,
  ) {
    ...
    switch (additionalBytes) {
      case (null) {};
      case (?bytes) {
        Buffer.writeMany(buffer, bytes);
      };
    };
  };

  private func encodeNatHeaderInternal(
    buffer : Buffer.Buffer<Nat8>,
    majorType : Nat8,
    value : Nat64,
  ) {
    let (additionalBits : Nat8, additionalBytes : ?Iter.Iter<Nat8>) = if (value <= 23) {
      (Nat8.fromNat(Nat64.toNat(value)), null);
    } else {
      let addBitsList = List.empty<Nat8>();
      let addBitsBuffer = Buffer.fromList(addBitsList);
      let additionalBits : Nat8 = if (value <= 0xff) {
        addBitsBuffer.write(Nat8.fromNat(Nat64.toNat(value)));
        24;
      } else if (value <= 0xffff) {
        NatX.toNat16BytesBuffer(addBitsBuffer, Nat16.fromNat(Nat64.toNat(value)), #msb);
        25 // 25 indicates 2 more bytes of info
      } else if (value <= 0xffffffff) {
        NatX.toNat32BytesBuffer(addBitsBuffer, Nat32.fromNat(Nat64.toNat(value)), #msb);
        26 // 26 indicates 4 more bytes of info
      } else {
        NatX.toNat64BytesBuffer(addBitsBuffer, value, #msb);
        27 // 27 indicates 8 more bytes of info
      };
      (additionalBits, ?List.values(addBitsList));
    };
